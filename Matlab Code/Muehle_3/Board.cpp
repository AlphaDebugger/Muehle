...
void Board::drop2(int i)
{
	...
	///
	/// Hier wird die Ausgabe abgegriffen
	///
    ...
    if(intersection_remplie[i]==1)
    {
        ...
        cout<<"XXXX";
        color(8,0);
        //cout << "Mensch hat auf " << i << "gesetzt" << endl;
        cout<<"";
    ...
    if(intersection_remplie[i]==-1)
    {
		...
		// �ffnen der Datei, in die geschrieben werden soll
		//ausgabe.open("ausgabeDatei");
    	//ausgabe << i << std::endl;
    	//ausgabe.close();
        color(11,11);
        cout<<"OOOO";
        color(8,0);
        //cout << "Computer hat auf " << i << " gesetzt" << endl;
        cout<<"";
    }
    if(intersection_remplie[i]==0)
    {
...
void Board::ingame()
{
    ...
    while(times<18)
    {
        system("@cls||clear");
        afficher_grille();
       cout<<"Anzahl der verarbeiteten Knoten "<<ai.nbr_noeuds<<endl;

        if(player==-1)
        {
            ai.nbr_noeuds=0;
            int chosen=ai.minimax1(intersection_remplie,0,-1,0,-100000000000,+100000000000); //to see about this true
            ...
            if(moulinformation(chosen,-1,intersection_remplie))
            {
                ...
                for(int i=0;i<24;i++)
                {
                    ...
                    {
                        vector<int> v;
                        nachbar(i,&v,intersection_remplie,1);
        	...
        	///
        {
        	cout << "Wo möchtest Du setzen?" << endl;
            cin>>input;
/*
        	std::ifstream eingabe("eingabeDatei");
            string eingabeString = "";
            eingabe >> eingabeString;
            std::stoi(eingabeString) = input;
*/
            intersection_remplie[input]=player;
            if(moulinformation(input,1,intersection_remplie))
            {
                int todelete=0;
                cout <<"Welcher Stein soll gelöscht werden?"<< endl;
    ...
    while(blanc!=2&&noir!=2)
    {
        ...
        {    int selected=0,destination=0;
            do
            {
                ...
                cout<<" Stein zum Verschieben w�hlen: "<<endl;
                do{
                    cin>>selected;
                }while(intersection_remplie[selected]!=1);
                cout<<"Ziel w�hlen: "<<endl;
                do{
                    cin>>destination;
            ...
            if(moulinformation(destination,1,intersection_remplie))
            {
                ...
                while(intersection_remplie[todelete]!=-1||moulinformation(todelete,-1,intersection_remplie))
                {
                    cout<<"Bitte anderen Stein wählen"<<endl;
                    cin>>todelete;
                }
                intersection_remplie[todelete]=0;
            }
            ...
            if(noir!=3)
           {
                ...
                int from=0;
                ai.nbr_noeuds=0;
                from=ai.minimaxphase2(intersection_remplie,0,-1,&to,0,intersection_remplie,-100000000000,+100000000000);
           ...
           if(noir==3)
           {
                ...
                int from=0;
                ai.nbr_noeuds=0;
                from=ai.minimaxphase3(intersection_remplie,0,-1,&to,0,intersection_remplie,-100000000000,+100000000000);
                intersection_remplie[from]=0;
                intersection_remplie[to]=-1;
                ////////////////////////////////////////////////////////
                if(moulinformation(to,-1,intersection_remplie))
...
void Board::nachbar(int i,vector<int>* v,int state[24],int with)
{
    switch(i){
case 0:
    {

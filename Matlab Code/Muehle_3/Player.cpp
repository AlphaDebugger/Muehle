...
int Player::minimax1(int state[24],int depth,int ismax,int in,int alpha, int beta)
{
    Board table(state);
    if(depth==1)
    {
        ...
        for(int i=0;i<24;i++)
        {
            if(state[i]==0)
            {
                ...
                for(int i=0;i<24;i++)
                {
                    ...
                    {
                        vector<int> v;
                        table.nachbar(i,&v,table.intersection_remplie,-1);
                ...
                //
                int son=minimax1(state,depth+1,-1,i,alpha,beta);
                fils.push_back(son);
                state[i]=0;
                scores[i]=son;
                nbr_noeuds++;
        ...
        for(int i=0;i<24;i++)
        {
            if(state[i]==1)
            {
                ...
                for (it = v.begin(); it != v.end(); it++)
                {
                    state[*it]=1;
                    state[i]=0;
                    int son=minimaxphase2(state,depth+1,-1,dest,*it,initial,alpha,beta);
                    fils.push_back(son);
                    state[i]=1;
                    state[*it]=0;
